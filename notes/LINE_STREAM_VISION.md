...
This vision has evolved through design exploration:

### [V1: Layered Architecture](ARCHITECTURE_V1_LAYERED.md)
**Approach:** 8-layer abstraction with specialized tools and coordinator layers
- Source Connectors, Stream Adapters, Line Buffers, Filters, Buffer Manager, Session Manager, Protocol, Client
- Built custom tools: `gbe-source-tail`, `gbe-filter-grep`, etc.
- Manager layers coordinate components

**Problem:** Too complex. Reimplements Unix tools. Coordinators violate "small tools" philosophy.

### [V2: Adapter-Centric Architecture](ARCHITECTURE_V2_ADAPTER_CENTRIC.md) ← **Current**
**Approach:** 4 simple tools + existing Unix utilities
...
**Key insight:** Use existing Unix tools (`tail`, `grep`, `bash`) via adapter. No coordinators—just message passing.
...
Everything—terminals, logs, files, databases, commands—flows through the same pattern:
...
gbe-client       Render + input
gbe-buffer       (Optional) Storage for seeking/editing
```
...
**Database:**
```bash
gbe-adapter "psql -c 'SELECT * FROM users'"
```

### Why This Works

- **Use existing tools:** `tail`, `grep`, `bash`, `psql` instead of reimplementing
- **No coordinators:** Router just forwards messages
- **True Unix:** Small tools composed via protocol
- **Universal:** Any command works immediately

---

## Full Documentation

**→ [V2: Adapter-Centric Architecture](ARCHITECTURE_V2_ADAPTER_CENTRIC.md)** (Current design)
- Complete architecture
- Protocol definitions
- Use cases
- Implementation plan

**→ [V1: Layered Architecture](ARCHITECTURE_V1_LAYERED.md)** (Historical)
...
## V1 Content (Archived Below)

The content below is from V1 and preserved for reference. See ARCHITECTURE_V2_ADAPTER_CENTRIC.md for current design.
